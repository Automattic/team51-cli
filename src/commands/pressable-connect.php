...
class PressableConnect extends Command {
	protected static $defaultName = 'pressable-connect';

	protected function configure() {
		$this
		->setDescription( 'Retrieves SFTP Username for a Pressable site. This command can also connect you directly via SSH or SFTP.' )
		->setHelp( 'When using this, your SSH key should be set, and you should only access the command using your own email address.' )
		->addArgument( 'site-domain', InputArgument::REQUIRED, 'Site domain/URL (e.g. asia.si.edu).' )
		->addOption( 'email', null, InputOption::VALUE_REQUIRED, 'The email address to use to access the SSH account' )
		->addOption( 'connect', 'c', InputOption::OPTIONAL, 'Must be either "ssh" or "sftp". If set, it will automatically connect you via ssh or sftp' );
	}
	...
	protected function execute( InputInterface $input, OutputInterface $output ) {
		...
		if ( empty( $input->getOption( 'issue-repo-only' ) ) ) {
			$response = $api_helper->call_github_api(
				'orgs/' . GITHUB_API_OWNER . '/repos',
				array(
					'name'             => $slug,
